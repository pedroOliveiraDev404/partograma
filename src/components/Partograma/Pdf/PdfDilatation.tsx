import { useEffect, useState } from "react";
import { useSelector } from "react-redux";
import { ObjData } from "../../../store/modules/partogramaChart/types";

const PdfDilatation = () => {
  const [events, setEvents] = useState<ObjData[]>([]);
  const { positionData } = useSelector(
    (state: Store.State) => state.partogramaChart.data
  );

  useEffect(() => {
    if (positionData) {
      setEvents(positionData.filter((item) => item.dilatacao));
    }
  }, [positionData]);

  return (
    <>
      <strong
        style={{
          marginTop: "24px",
          marginBottom: "24px",
        }}
        className="export-pdf-strong"
      >
        Cervicodilatação
      </strong>
      {events.map((item) => (
        <div className="row-container">
          <div className="column-container">
            <span>{`Data e hora: ${new Date(item.time as Date)
              .toLocaleString("pt-BR")
              .slice(0, 17)}`}</span>
            <span> {`Dilatação: ${item.dilatacao}.`}</span>
            <span> {`Esvaecimento: ${item.fading}.`}</span>
            <span> {`Consistência: ${item.consistency}.`}</span>
            <span> {`Posição: ${item.positionDilatation}.`}</span>
          </div>
          <div className="column-container">
            <span>
              {`Data e horário de registro: ${new Date()
                .toLocaleString("pt-BR")
                .slice(0, 17)}.`}
            </span>
            <span>Registrado por X.</span>
            {item.observation && (
              <span> {`Observação: ${item.observation}.`}</span>
            )}
          </div>
        </div>
      ))}
    </>
  );
};

export default PdfDilatation;
